...
def validate(model, opt, data_loader=None):
    if data_loader is None:
        data_loader, _ = create_dataloader(opt, lmdb_path=opt.lmdb_path, is_train=False, distributed=getattr(opt, 'distributed', False))
    ...
    with torch.no_grad():
        ...
        progress_bar = tqdm(total=len(data_loader), desc="Validating", dynamic_ncols=True, disable=not show_progress)
