...
if __name__ == '__main__':
    ...
    if opt.train_lmdb is None and opt.dataroot:
    ...
    val_path = None
    if opt.val_lmdb:
        val_path = opt.val_lmdb
    elif opt.train_lmdb is None and opt.dataroot:
        candidate = f'{opt.dataroot}/{opt.val_split}/'
        if os.path.exists(candidate):
            val_path = candidate

    if val_path:
        val_opt = build_val_opt(opt)
        if val_path.endswith('.lmdb'):
            val_opt.val_lmdb = val_path
        else:
            val_opt.dataroot = val_path
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        if val_loader is not None:
            ...
            if opt.rank == 0 and metrics is not None:
                ...
                print(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} (Val @ epoch {epoch}) acc: {metrics['acc']}; pr_auc: {metrics['pr_auc']}; auroc: {metrics['auroc']}; avg_recall: {metrics['avg_recall']}")
