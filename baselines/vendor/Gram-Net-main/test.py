...
if __name__ == '__main__':
    ...
    model.load_state_dict(state_dict['model'] if isinstance(state_dict, dict) and 'model' in state_dict else state_dict)
    model.to(opt.device)
    model.eval()
    ...
    for v_id, (lmdb_path, name) in enumerate(zip(lmdb_paths, eval_names)):
        ...
        metrics, _, _ = validate(model, eval_opt, lmdb_path=lmdb_path, distributed=False)
