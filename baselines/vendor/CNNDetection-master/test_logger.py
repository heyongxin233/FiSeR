...
def log_test_run(project_root, project_name, dataset_path, ckpt_path, metrics, cmd=None, extra=None):
    try:
        ...
        logs_dir = Path(os.environ.get("BASELINE_LOG_DIR", root / "logs"))
        ...
        with (logs_dir / "logs.txt").open("a", encoding="utf-8") as f:
            f.write("\n".join(lines))
        ...
        with (logs_dir / "logs.jsonl").open("a", encoding="utf-8") as f:
