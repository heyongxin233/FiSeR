"""Metric utilities for AIDE fine-tuning workflows."""
...
def evaluate_metrics(test_labels, y_score, threshold_param: float = -1, target_fpr: float = 0.05):
    """Compute common classification metrics from logits or probabilities.

    Args:
        test_labels: Array-like of ground-truth binary labels (0/1).
        y_score: Array-like of predicted scores or probabilities for the positive class.
        threshold_param: If ``-1``, select the threshold that maximizes F1. Otherwise, use the
            provided threshold value directly.
        target_fpr: False positive rate at which to report the corresponding true positive rate.

    Returns:
        A dictionary containing AUROC, PR-AUC, F1, precision, recall, accuracy, average recall,
        positive/negative recall, the threshold used, and TPR at the specified FPR.
    """
