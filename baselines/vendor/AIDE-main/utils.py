...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if args.resume:
        if args.resume.startswith('https'):
            ...
            # PyTorch 2.6 defaults to weights_only=True, which cannot load full training
            # checkpoints saved with metadata such as argparse.Namespace. Explicitly
            # disable weights_only to remain compatible with existing checkpoints.
...
def adjust_learning_rate(optimizer, global_step, total_steps, args):
    ...
    warmup_steps = min(args.warmup_steps, args.iters_per_epoch)
