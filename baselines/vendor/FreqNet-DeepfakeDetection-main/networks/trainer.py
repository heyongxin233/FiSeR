...
class Trainer(BaseModel):
    ...
    def __init__(self, opt):
        ...
        def _cosine_warmup(step_idx: int, total_steps: int) -> float:
            warmup_steps = min(opt.warmup_steps, opt.iters_per_epoch)
        ...
        if self.isTrain:
            ...
            self.total_train_steps = opt.niter * getattr(opt, "iters_per_epoch", 1)
            ...
            if opt.warmup_steps > 0:
                ...
                self.lr = self.current_learning_rate()

        if not self.isTrain or opt.continue_train:
            self.load_networks(opt.epoch)
        self.model.to(opt.device if hasattr(opt, "device") else opt.gpu_ids[0])
    ...
    def current_learning_rate(self):
        ...
        if self.optimizer and self.optimizer.param_groups:
            return self.optimizer.param_groups[0].get('lr', None)
        return None
