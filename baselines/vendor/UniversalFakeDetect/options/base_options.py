...
def normalize_arch(arch):
    if arch is None:
        return arch
    if ':' in arch:
        prefix, name = arch.split(':', 1)
        if prefix.lower() == "imagenet":
            return f"Imagenet:{name}"
        if prefix.lower() == "clip":
            return f"CLIP:{name}"
        return arch
    ...
    alias_map = {
        ...
        "vgg11": "Imagenet:vgg11",
        "vgg19": "Imagenet:vgg19",
        "swin-t": "Imagenet:swin-t",
        "swin-s": "Imagenet:swin-s",
        "swin-b": "Imagenet:swin-b",
        ...
        "rn101": "CLIP:RN101",
        "rn50x4": "CLIP:RN50x4",
        "rn50x16": "CLIP:RN50x16",
        "rn50x64": "CLIP:RN50x64",
        "vit-b/32": "CLIP:ViT-B/32",
        "vit-b/16": "CLIP:ViT-B/16",
        "vit-l/14": "CLIP:ViT-L/14",
        "vit-l/14@336px": "CLIP:ViT-L/14@336px",
    }
    return alias_map.get(key, arch)
...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument(
            ...
            help='model backbone (e.g., CLIP:ViT-L/14, Imagenet:resnet50)',
        )
    ...
    def parse(self, print_options=True):
        ...
        opt.arch = normalize_arch(opt.arch)
