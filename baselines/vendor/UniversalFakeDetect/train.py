...
if __name__ == '__main__':
    ...
    if opt.rank == 0:
        tqdm.write(f'cwd: {os.getcwd()}')
        tqdm.write(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} Length of data loader: {len(train_loader)}")
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        if epoch % opt.save_epoch_freq == 0 and opt.rank == 0:
            tqdm.write(f'saving the model at the end of epoch {epoch}')
